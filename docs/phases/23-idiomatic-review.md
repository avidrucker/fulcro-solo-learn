...
- ✅ [23.3 — Prioritized recommendations](23-3-recommendations.md) — 4 candidate items written up as Phase 24+ stories with effort × value bucketing. **Revised headline (post-correction): no high-value refactor candidates surface from this audit.** The original #1 (Phase 24 CI workflow) was withdrawn after the doc shipped — `.github/workflows/main.yml` already runs the test suite on every push to `main`; the recommendation was based on incomplete recon. See the doc's CORRECTION section at the top. Items #2 (`m/returning`) and #3 (Guardrails audit) remain as small discretionary cleanups; #4 (namespace split) deferred until a second entity exists. The lesson from the analytical miss is captured as Mistake C12 in [`learned_while_making_this.md`](../learned_while_making_this.md).
